<x-app-layout>
                                    ...
                                    <div class="border-b border-gray-200 mb-2">
                                        ...
                                        <div class="">
                                            ...
                                            <div class="block sm:hidden w-full">
                                                <select id="process-tabs-mobile" class="w-[395px] border p-2 rounded">
                                                    @php
                                                        $processCounter = 0;
                                                    ...
                                                    @foreach($questions as $question)
                                                        @if ($question->processDetails->process_name != $previousProcess)
                                                            ...
                                                            @endphp
                                                            <option value="process{{ $processCounter }}">
                                            ...
                                            {{-- Desktop tabs --}}
                                            <ul class="hidden sm:flex flex-nowrap -mb-px text-sm font-medium text-center" id="process-tabs" data-tabs-toggle="#process-tab-content" role="tablist">
                                                @php
                                                    $processCounter = 0;
                                                    $previousProcess = null;
                                                @endphp
                                                @foreach($questions as $question)
                                                    ...
                                                    @if ($question->processDetails->process_name != $previousProcess)
                                                        ...
                                                        <li class="me-2" role="presentation">
                                                            <button class="inline-block p-2 border-b-2 rounded-t-lg whitespace-nowrap" id="process{{$processCounter}}-tab" data-tabs-target="#process{{$processCounter}}" type="button" role="tab" aria-controls="process{{$processCounter}}" aria-selected="false">
                                                                {{ $question->processDetails->process_name }}
                                                            </button>
                                                        </li>
                                                    @endif
    ...
    <script>
        $(document).ready(function () {
            ...
            const select = document.getElementById('process-tabs-mobile');

            if (select) {
                select.addEventListener('change', function () {
                    const selectedValue = this.value;
                    const targetTabButton = document.querySelector(`#${selectedValue}-tab`);
                    if (targetTabButton) {
                        targetTabButton.click(); // Simulate tab click
                    }
                });
            }
